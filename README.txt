...
- Index + cache: photo_catalog.json, thumbs/ (created at runtime)
...
1) Open Command Prompt and go to the project folder:
   cd /d "C:\vivagoal-photo-finder"  (use your actual path)
...
Incremental Rescans
- The first scan builds `photo_catalog.json`. Subsequent rescans re-use existing entries and only rebuild items whose size/modified time changed, so they finish much faster than the initial full scan.
...
- Python 3.14: Pillow wheels aren’t available yet; the grid serves originals. EXIF dates read using pure-Python exifread.

Troubleshooting
- start_server.bat not recognized in PowerShell → prefix with .\
  .\start_server.bat 5000
- Browser doesn’t open automatically → navigate to http://localhost:5000
